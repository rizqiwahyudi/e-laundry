...
$pdf->Text(13, 5.8, 'LAPORAN BELUM DI AMBIL');

$pdf->SetMargins(3.2,3,1,1);
...
$pdf->Cell(3.2, -1, 'Pembayaran', 1, 0, 'C');
$pdf->Cell(3.1, -1, 'Jumlah Bayar', 1, 0, 'C');

$no=1;
$qu = mysqli_query($conn, "SELECT * FROM transaksi left join master on master.id_master=transaksi.id_master left join karyawan on karyawan.id_karyawan=transaksi.id_karyawan left join konsumen on konsumen.nama=transaksi.nama_R where transaksi.status_pengambilan = 'Belum di ambil' and transaksi.tgl_transaksi between '$a' and '$b' order by nama_karyawan ASC");
while ($d = mysqli_fetch_array($qu)) {
	...
	$pdf->Cell(3.2, -1, $d['status_pembayaran'], 1, 0, 'C');
	$pdf->Cell(3.1, -1, 'Rp.'.number_format($d['jumlah_bayar']), 1, 0, 'C');
	$no++;
}
$pdf->ln(1);
$pdf->Cell(24.7, -1, 'Total ', 1, 0, 'C');
$pdf->Cell(3.1, -1, 'Rp.'.number_format($da['jml']), 1, 0, 'C');
